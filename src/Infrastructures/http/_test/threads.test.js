...
describe("/threads endpoint", () => {
  ...
  it("should response 201 and new thread", async () => {
    ...
    expect(responseJsonThread.data.addedThread.title).toEqual(
      requestAddThread.title
    );
  });
  ...
  it("should response 400 when prop does not exist", async () => {
    ...
    expect(responseJsonThread.message).toEqual(
      "menambahkan thread gagal karena properti yang dibutuhkan tidak ada"
    );
  });
  ...
  it("should response 200 when get detail thread", async () => {
    ...
    expect(responseJsonGetDetailThread.data.thread.comments[1].content).toEqual(
